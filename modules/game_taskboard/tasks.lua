...
-- Open a secondary HTML modal without clobbering self.ui/self.htmlId.
-- Returns { htmlId, ui } which callers store and later pass to closeModal().
function TaskBoardController:openModal(path)
    local suffix = ".html"
...
function TaskBoardController:closeModal(modal)
    if not modal then return end
    if modal.htmlId ~= nil then
        g_html.destroy(modal.htmlId)
        modal.htmlId = nil
    end
    if modal.ui then
        if modal.ui.destroy and not modal.ui:isDestroyed() then
            pcall(function() modal.ui:destroy() end)
        end
        modal.ui = nil
    end
end
