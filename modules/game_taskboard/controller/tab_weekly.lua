...
function TaskBoardController:onWeeklyServerData(header, monsters, items)
    ...
    -- Difficulty modal.
    --
    -- Simple rule verified against the official Tibia client: the popup
    -- must appear whenever there are no active kill/delivery tasks for
    -- the week — the user can't make progress on a week without tasks,
    -- so Tibia blocks the UI behind the difficulty picker until they
    -- choose one.
    --
    -- Previously we also checked `weeklyProgressFinished`, but the server
    -- keeps that flag set from last week's completed progress even after
    -- the new week rolls over, so it suppressed the popup at exactly the
    -- moment it was needed. The tasks/no-tasks state is the only reliable
    -- trigger.
    local hasAssignedTasks = (#monsterList > 0) or (#itemList > 0)
    self.weeklyDifficultyPending = not hasAssignedTasks
